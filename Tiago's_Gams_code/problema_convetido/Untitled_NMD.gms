...
*Sets i;
$include "C:\Users\TA\Desktop\problems QCQP\problema_convetido\teste_sets.inc";
alias(i,j);
...
$include "C:\Users\TA\Desktop\problems QCQP\problema_convetido\teste.inc";

Positive Variable x(i);
Positive Variable w(i,j);
Binary Variable z(j,l,k);
Positive Variable hat_x(i,j,l,k);
...
Def_w(i,j)$sum(r,BL(r,i,j))..
         w(i,j) =E= (x.up(j)-x.lo(j))*sum((l,k)$(ord(l)<=-p),10**(-ord(l))*hat_x(i,j,l,k)*(ord(k)-1)) + (x.up(j)-x.lo(j))*v(i,j) + x(i)*x.lo(j);

Def_x(j)$sum((r,i),BL(r,i,j))..
         x(j) =E= (x.up(j)-x.lo(j))*sum((l,k)$(ord(l)<=-p),10**(-ord(l))*z(j,l,k)*(ord(k)-1)) + (x.up(j)-x.lo(j))*delta_x(j) + x.lo(j);
...
UB_prod(i,j,l,k)$(sum(r,BL(r,i,j)) and ord(l)<=-p)..
         x(i) - hat_x(i,j,l,k) =L= x.up(i)*(1-z(j,l,k));

LB_prod(i,j,l,k)$(sum(r,BL(r,i,j)) and ord(l)<=-p)..
         x(i) - hat_x(i,j,l,k) =G= x.lo(i)*(1-z(j,l,k));

dis_1(i,j,l)$(sum(r,BL(r,i,j)) and ord(l)<=-p)..
         sum(k, hat_x(i,j,l,k)) =E= x(i);

dis_2(j,l)$(sum((r,i),BL(r,i,j))and ord(l)<=-p)..
         sum(k, z(j,l,k)) =E= 1;
...
MC_3(i,j)$sum(r,BL(r,i,j))..
         v(i,j) =G= (10**p)*(x(i)-x.up(i)) + delta_x(j)*x.up(i);

MC_4(i,j)$sum(r,BL(r,i,j))..
         v(i,j) =L= (10**p)*(x(i)-x.lo(i)) + delta_x(j)*x.lo(i);

* Bounds das variáveis
delta_x.up(j)$sum((r,i),BL(r,i,j)) = 10**p;
x.lo(i) = LB_x(i);
x.up(i) = UB_x(i);
