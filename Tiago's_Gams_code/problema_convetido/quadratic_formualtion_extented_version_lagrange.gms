...
R_FO(r)$(re_FO(r))..
         sum((i,j)$BL(r,i,j),a(r,i,j)*x(i)*x(j)) + sum(i,b(r,i)*x(i)) + sum(r1$(not re_FO(r1)), mult(r1)*( sum((i,j)$BL(r1,i,j),a(r1,i,j)*x(i)*x(j)) + sum(i,b(r1,i)*x(i)) + c(r1))) + (10)*sum(i, power(x(i),2)) - (10)*sum(i, w(i)) =E= FO;
...
*         mult(r1) = 0;
         ...
         while (not stop_2,
                 ...
                 LB = max(LB,FO_atual);
                 display FO_atual;
                 subg(r1)$(not re_FO(r1)) = sum((i,j)$BL(r1,i,j),a(r1,i,j)*x.l(i)*x.l(j)) + sum(i,b(r1,i)*x.l(i)) + c(r1);
