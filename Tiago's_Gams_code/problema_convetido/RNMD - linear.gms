...
MC_w_1(i,j)$sum(r,BL(r,i,j))..
         w(i,j) =L= x.up(i)*x(j) + x.lo(i)*x(j) - x.up(i)*x.lo(j);

MC_w_2(i,j)$sum(r,BL(r,i,j))..
         w(i,j) =L= x.up(j)*x(i) + x.lo(j)*x(i) - x.up(j)*x.lo(i);

MC_w_3(i,j)$sum(r,BL(r,i,j))..
         w(i,j) =G= x.up(i)*x(j) + x.up(j)*x(i) - x.up(i)* x.up(j);

MC_w_4(i,j)$sum(r,BL(r,i,j))..
         w(i,j) =G= x.lo(i)*x(j) + x.lo(j)*x(i) - x.lo(i)*x.lo(j);
...
p = -5;
delta_x.up(j)$sum((r,i),BL(r,i,j)) = 2**p;
v.up(i,j)$sum(r,BL(r,i,j)) = 4**p;

scalar time_Start, time_current, time_limit;
time_limit = 3600.0*24;
time_Start = TimeElapsed;
Rel.reslim = time_limit;
