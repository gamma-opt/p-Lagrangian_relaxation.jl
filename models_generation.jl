...
function original_problem_generation(number_of_scenarios, number_of_continuous_decision_variables, number_of_integer_decision_variables, number_of_constrains, Qdensity)
    ...
    @constraint(original_problem, [s = 1 : number_of_scenarios, i = 1 : number_of_constrains ],
        sum( x[j, s]' * constraint_Qs[s][i][j,k] * x[k, s] for j = 1 : number_of_continuous_decision_variables, k = 1: number_of_continuous_decision_variables)
        + sum( x[j, s] * constraint_fs[s][i][1, j] for j = 1 : number_of_continuous_decision_variables)
    ...
    @constraint( original_problem, [s in 2 : number_of_scenarios, i = 1 : number_of_continuous_decision_variables],
        x[i, s] - x[i, 1] == 0 )
...
function RNMDT_problem_generation(p, number_of_scenarios, number_of_continuous_decision_variables, number_of_integer_decision_variables, number_of_constrains, Qdensity)
    ...
    @variables RNMDT_problem begin
        #RNMDT variables
        x_heat_RNMDT[ 1 : number_of_continuous_decision_variables, 1 : number_of_continuous_decision_variables, p : -1, 1 : number_of_scenarios ] # x_heat
        delta_x_RNMDT[ 1 : number_of_continuous_decision_variables, 1 : number_of_scenarios ] # delta_x_heat
        z_RNMDT[ 1 : number_of_continuous_decision_variables, p : -1, 1 : number_of_scenarios ], Bin
    ...
    @constraint( RNMDT_problem, [ s = 1 : number_of_scenarios, r = 1 : number_of_constrains ],
        ...
        + 2 * sum(constraint_Qs[s][r][i, j] * w_RNMDT[i, j, s]
            for i = 1 : number_of_continuous_decision_variables,
            j = i+1 : number_of_continuous_decision_variables)
        + sum( x[i, s] * constraint_fs[s][r][1, i] for i = 1:number_of_continuous_decision_variables)
    ...
    @constraint( RNMDT_problem, [ i = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables, l = p : -1, s = 1 : number_of_scenarios],
    ...
    # non-anticipativity condition from original problem 4-5
    @constraint( RNMDT_problem, [s in 2 : number_of_scenarios, i = 1 : number_of_continuous_decision_variables],
        x[i, s] - x[i, 1] == 0 )
...
function dynamic_precision_RNMDT_problem_generation(precision_p, number_of_scenarios, number_of_continuous_decision_variables, number_of_integer_decision_variables, number_of_constrains, Qdensity)
    ...
    @variables RNMDT_problem begin
        #RNMDT variables
        x_heat_RNMDT[ 1 : number_of_continuous_decision_variables, 1 : number_of_continuous_decision_variables, minimum(precision_p) : -1, 1 : number_of_scenarios ] # x_heat
        delta_x_RNMDT[ 1 : number_of_continuous_decision_variables, 1 : number_of_scenarios ] # delta_x_heat
    ...
    @constraint( RNMDT_problem, [ i = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables, s = 1 : number_of_scenarios],
        2.0^precision_p[i] * ( x[i, s] - x_boundaries[i, 2] ) + x_boundaries[i, 2] * delta_x_RNMDT[j, s] <= delta_w_RNMDT[i, j, s]) # 31

    @constraint( RNMDT_problem, [ i = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables, s = 1 : number_of_scenarios],
        delta_w_RNMDT[i, j, s] <= 2.0^precision_p[i] * ( x[i, s] - x_boundaries[i, 1] ) + x_boundaries[i, 1] * delta_x_RNMDT[j, s]) # 31
    ...
    @constraint( RNMDT_problem, [ i = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables, l = precision_p[j] : -1, s = 1 : number_of_scenarios],
        x_boundaries[i, 1] * z_RNMDT[j, l, s]  <= x_heat_RNMDT[i, j, l, s]) # 33
...
function LD_RNDMT_problem_generation(p, number_of_scenarios, number_of_continuous_decision_variables, number_of_integer_decision_variables, number_of_constrains, Qdensity)
    ...
    for s = 1 : number_of_scenarios
        ...
        # continuous decision variables
        @variable( subproblem[s], x[1 : number_of_continuous_decision_variables] )
        ...
        @variables subproblem[s] begin
            ...
            #RNMDT variables
            x_heat_RNMDT[ 1 : number_of_continuous_decision_variables, 1 : number_of_continuous_decision_variables, p : -1 ] # x_heat
            delta_x_RNMDT[ 1 : number_of_continuous_decision_variables ] # delta_x_heat
            z_RNMDT[ 1 : number_of_continuous_decision_variables, p : -1 ], Bin
        ...
        @objective( subproblem[s], Max,
            ...
            + 2 * sum(objective_Qs[s][i, j] * w_RNMDT[i, j]
                for i = 1 : number_of_continuous_decision_variables,
                    j = i+1 : number_of_continuous_decision_variables)
            + sum( x[i] * objective_fs[s][1, i]  for i = 1:number_of_continuous_decision_variables)
        ...
        @constraint( subproblem[s], [ r = 1 : number_of_constrains ],
            ...
            + 2 * sum(constraint_Qs[s][r][i, j] * w_RNMDT[i, j]
                for i = 1 : number_of_continuous_decision_variables,
                j = i+1 : number_of_continuous_decision_variables)
            + sum( x[i] * constraint_fs[s][r][1, i] for i = 1:number_of_continuous_decision_variables)
        ...
        @constraint( subproblem[s], [ i = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables, l = p : -1],
...
function dynamic_precision_based_LD_RNDMT_problem_generation(precision_p, number_of_scenarios, number_of_continuous_decision_variables, number_of_integer_decision_variables, number_of_constrains, Qdensity)
    ...
    for s = 1 : number_of_scenarios
        ...
        @variables subproblem[s] begin
            ...
            #RNMDT variables
            x_heat_RNMDT[ 1 : number_of_continuous_decision_variables, 1 : number_of_continuous_decision_variables, minimum(precision_p) : -1 ] # x_heat
            delta_x_RNMDT[ 1 : number_of_continuous_decision_variables ] # delta_x_heat
        ...
        @constraint( subproblem[s], [ j  = 1 : number_of_continuous_decision_variables],
            x[j] == (x_boundaries[j, 2] - x_boundaries[j, 1])  *
            ( sum( 2.0^l * z_RNMDT[j, l] for l = precision_p[j] : -1) + delta_x_RNMDT[j] ) ) # 28

        @constraint( subproblem[s], [ i  = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables ],
            w_RNMDT[i, j] == (x_boundaries[j, 2] - x_boundaries[j, 1]) *
            ( sum( 2.0^l * x_heat_RNMDT[i, j, l] for l = precision_p[j] : -1) + delta_w_RNMDT[i, j] ) ) # 29

        @constraint( subproblem[s], [ j  = 1 : number_of_continuous_decision_variables ],
            0 <= delta_x_RNMDT[j] <= 2.0^precision_p[j] ) # 30

        @constraint( subproblem[s], [ i = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables],
            2.0^precision_p[i] * ( x[i] - x_boundaries[i, 2] ) + x_boundaries[i, 2] * delta_x_RNMDT[j] <= delta_w_RNMDT[i, j]) # 31

        @constraint( subproblem[s], [ i = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables],
            delta_w_RNMDT[i, j] <= 2.0^precision_p[i] * ( x[i] - x_boundaries[i, 1] ) + x_boundaries[i, 1] * delta_x_RNMDT[j]) # 31
        ...
        @constraint( subproblem[s], [ i = 1 : number_of_continuous_decision_variables, j = 1 : number_of_continuous_decision_variables, l = precision_p[j] : -1],
            x_boundaries[i, 1] * z_RNMDT[j, l]  <= x_heat_RNMDT[i, j, l]) # 33
