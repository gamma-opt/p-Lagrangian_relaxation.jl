...
function dynamic_precision_based_Lagrangian_decomposition_bundle(precision_p, number_of_scenarios, number_of_integer_decision_variables, number_of_continuous_decision_variables, number_of_constrains, Qdensity, number_of_iterations, center_of_gravity_inital_value)
    ...
    eps_stop = 1000 # stopping criteria
    number_of_iteration_for_checking = 2

    #iteration_time = SharedArray{Float64}(1, number_of_iterations)

    initial_time = time()
    while (iteration < number_of_iterations) & ((iteration > number_of_iteration_for_checking + 1 ) ? (norm(dual_objective_value_at_lagrangian[iteration - number_of_iteration_for_checking : iteration-1].- dual_function_value_at_the_center_of_gravity[iteration - number_of_iteration_for_checking - 1 : iteration - 2])^2 >= eps_stop) : true)
    #while (iteration < number_of_iterations) & ((iteration > number_of_iteration_for_checking+1) ? (auxiliary_check(vector_of_lambda_lagrangian, iteration, number_of_iteration_for_checking, number_of_scenarios, eps_stop) != 0 ) : true)
        ...
        @suppress @sync @distributed for s in 1 : number_of_scenarios
            #try_time = time()
            #objective_update
        ...
        @constraint(cutting_plane_subproblem, z >= dual_function_value_at_the_center_of_gravity[iteration] + sum( sum( subgradient_vector[iteration, s] .* ( cutting_plane_subproblem[:lagrangian_multipliers_representing_variable][:, s] .- vector_of_lambda_lagrangian[iteration, s] ) ) for s = 1 : number_of_scenarios - 1) )
