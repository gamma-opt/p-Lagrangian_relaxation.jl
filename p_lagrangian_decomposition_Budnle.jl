...
function dynamic_precision_based_Lagrangian_decomposition_bundle(precision_p, number_of_scenarios, number_of_integer_decision_variables, number_of_continuous_decision_variables, number_of_constrains, Qdensity, number_of_iterations)
    ...
    lagrangian_multipliers_max = 100
    ...
    integer_decision_variables_values_for_each_scenario = SharedArray{Float64}(number_of_integer_decision_variables, number_of_scenarios)

    # vector that contains decision variables written in a column (x and y in this case)
    # (each row represnets the components of the correspondent lagrangian lagrangian_multipliers_representing_variableltiplier)
    continuous_decision_variables_values_for_each_scenario = SharedArray{Float64}(number_of_continuous_decision_variables, number_of_scenarios)
    ...
    subproblems = dynamic_precision_based_LD_RNDMT_problem_generation(precision_p, number_of_scenarios, number_of_integer_decision_variables, number_of_continuous_decision_variables, number_of_constrains, Qdensity)
    ...
    eps_stop = 1e-8 # stopping criteria
    initial_time = time()
    while (iteration < number_of_iterations) #& ((iteration>6) ? (norm(dual_objective_value_at_lagrangian[iteration - 5:iteration-1].- dual_objective_value_at_lagrangian[iteration-6:iteration-2])^2 >= eps_stop) : true)
        ...
        @sync @distributed for s in 1 : number_of_scenarios
            ...
            @objective( subproblems[s], Max,
                round( (1/number_of_scenarios), digits = 2) *
    ...
    elseif  dual_fiunction_value_at_the_center_of_gravity[iteration-1] - dual_objective_value_at_lagrangian[iteration] >= m * ( dual_fiunction_value_at_the_center_of_gravity[iteration-1] -  ( relaxed_dual_objective_value[iteration-1] + d * sum( sum( (vector_of_lambda_lagrangian[iteration, s] .- center_of_gravity[iteration-1, s]) .^ 2 )  for s = 1 : number_of_scenarios - 1 ) ) )
        ...
        @objective(cutting_plane_subproblem, Min, z + d * sum( sum( (cutting_plane_subproblem[:lagrangian_multipliers_representing_variable][:, s] .- center_of_gravity[iteration, s] ).^2 ) for  s in 1 : number_of_scenarios - 1 ) )

        @constraint(cutting_plane_subproblem, z >= dual_fiunction_value_at_the_center_of_gravity[iteration] + sum( sum( subgradient_vector[iteration, s] .* ( cutting_plane_subproblem[:lagrangian_multipliers_representing_variable][:, s] .- vector_of_lambda_lagrangian[iteration, s] ) ) for s = 1 : number_of_scenarios - 1) )
        ...
        print("value of the dual function at the center of gravity = $(dual_fiunction_value_at_the_center_of_gravity[iteration])\n \n")
    ...
    return dual_objective_value_at_lagrangian[ iteration ], integer_decision_variables_values_for_each_scenario[:, :], continuous_decision_variables_values_for_each_scenario[:, :], RNMDT_quadraticity_variables_w_for_each_scenario, vector_of_lambda_lagrangian[ 1 : iteration, :], number_of_the_serious_steps
